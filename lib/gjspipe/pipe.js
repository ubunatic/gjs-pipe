...
var Pipe = class Pipe {
     ...
     * let cancel = p.start(onResult, onError)
     * // ...
     * let err = cancel()  // You must call cancel later to avoid zombies!
     * 
     * @example
     * // start a simple command and read all output
     * let p = new Pipe('bash', '-c', 'echo 1').configure({read_timeout_ms: 100})
     * p.start((l) => print(l), null)
     * 
     */
    constructor(...cmd) {
