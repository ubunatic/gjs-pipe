...
/** returns a new `Promise` to be resolved or rejected with the result or error
 *  produced by first calling `async_fn` asynchronously and then handling the async
 *  result with `finish_fn`.
 * 
 * @example
 * ok = await glibAsync(
 *     (finish) => proc.wait_check_async(null, finish),  // passthrough glibAsync internal finish func
 *     (_, res) => proc.wait_check_finish(res),          // handle all GLib async results in own finish func
 * )
 */
function glibAsync(async_fn, finish_fn) {
    return new Promise((resolve, reject) => {
        async_fn((...args) => {
        ...
        const finish = (pipe_error=null) => {
            ...
            proc.wait_check_async(null, (_, res) => {
                ...
                if (termination_requested) {
                    ...
                    if (proc_error) onError(proc_error)
                    ok = ok && pipe_error == null && proc_error == null ? true : false
                }
                const exit_msg = `exit_status='${proc_error? proc_error.message : 'clean'}'`
...
module.exports = { Pipe, AGG_LINES, AGG_JSON, setTimeout, clearTimeout, asyncTimeout }
